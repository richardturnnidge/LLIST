...
start:                      ; Start code here
    ...
    ld hl, 9600
    ld (UART1_Struct), hl   ; set default baud to 9600


    ld a, (numParams)       ; number of params

    cp 2                    ; check if baud is included
    jr z, stdBaud           ; if no third param then assume default

                            ; work out baud rate from text entered

    ld ix, argv_ptrs        ; address of arument pointers
    ld hl, (ix+6)           ; address of first arg, is a 0 terminated string for the file

    push hl
    pop de                  ; put string pointer into DE

    call string2int         ; number is returned in HL
...
;    .dl     9600                ; baud rate 9600 in hex little endian, LSB first
    .dl     19200                ; baud rate 9600 in hex little endian, LSB first
    .db     8                   ; data bits
    .db     1                   ; stop bits
    .db     0                   ; parity bits
    .db     0                   ; flow control
